   ...
   ((and (not (eq first-char key-chord-last-unmatched))
    ...
    (let ((delay (if (key-chord-lookup-key
      ...
      (cond ((if executing-kbd-macro
               ...
               ;; Use read-event with timeout instead of sit-for.
               ;; sit-for runs redisplay/timers and stalls on heavy buffers.
               ;; read-event timeout returns nil immediately when no input,
               ;; avoiding the stall (key-seq style).
