...
;; Package-Version: 0.8.2-emacs30fix
...
Emacs 30+ compatible: avoids direct reference to read-key-empty-map
and read-key-full-map which changed behavior in Emacs 30.1."
  (and overriding-local-map
       (or (and (boundp 'read-key-empty-map)
                (eq overriding-local-map read-key-empty-map))
           (and (boundp 'read-key-full-map)
                (eq overriding-local-map read-key-full-map))
           ;; Emacs 30.1+: keymapにkey-chordプレフィクスが
           ;; 存在しなければread-char系の文脈とみなす
           (not (lookup-key overriding-local-map [key-chord])))))
